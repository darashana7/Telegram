...
class MinerviniScreener:
    ...
    def scan_stocks(self, symbols: List[str], min_score: int = 9) -> List[TrendTemplateResult]:
        """
        Scan multiple stocks using FAST batch processing.
        Uses batch download + concurrent processing for 10-20x speedup.
        ...
        results = []
        total = len(symbols)
        batch_size = 100  # Download 100 stocks at a time for max speed
        ...
        for batch_start in range(0, total, batch_size):
            ...
            logger.info(f"📥 Downloading batch {batch_start//batch_size + 1}/{(total + batch_size - 1)//batch_size} ({len(batch_symbols)} stocks)...")
            
            # Batch download historical data
            hist_data = self.data_fetcher.batch_download_stocks(batch_symbols)
            
            if not hist_data:
                logger.warning(f"No data for batch starting at {batch_start}")
                continue
            ...
            def process_stock(symbol):
                try:
                    ...
                    # Get stock info from pre-downloaded data
                    info = self.data_fetcher.get_stock_info_from_hist(symbol, hist)
                    if not info:
                        return None
                    
                    # Check trend template using the data
                    result = self._check_trend_template_fast(symbol, info, hist)
            ...
            with ThreadPoolExecutor(max_workers=20) as executor:
                futures = {executor.submit(process_stock, symbol): symbol for symbol in batch_symbols}
                
                for future in as_completed(futures):
                    result = future.result()
                    if result:
                        results.append(result)
                        logger.info(f"✅ {result.symbol} PASSES with score {result.score}/9")
        
        elapsed = time.time() - start_time
        logger.info(f"⏱️ Scan completed in {elapsed:.1f}s ({total/elapsed:.1f} stocks/sec)")
    ...
    def _check_trend_template_fast(self, symbol: str, info: Dict, hist: pd.DataFrame) -> Optional[TrendTemplateResult]:
        """
        Fast version of trend template check using pre-downloaded data.
        ...
        try:
            ...
            logger.debug(f"Error in fast check for {symbol}: {e}")
            return None
